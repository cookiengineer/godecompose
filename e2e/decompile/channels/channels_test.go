...
func TestChannels(t *testing.T) {
	b := decompile.CompileAndOpen(t, "channels")
	r := decompile.Decompile(t, b)

	t.Logf("output: %d bytes, instructions: %d", len(r.Output), len(r.Instructions))
	decompile.AssertPipelineOk(t, r, "channels")

	for _, m := range r.Matches {
		if decompile.HasMatch([]matcher.Match{m}, "chansend") || decompile.HasMatch([]matcher.Match{m}, "chanrecv") || decompile.HasMatch([]matcher.Match{m}, "fmt.") {
